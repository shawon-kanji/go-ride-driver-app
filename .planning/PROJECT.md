...
- ~~Known backend gap (blocking): JWT `aud` mismatch~~ — **Resolved, confirmed 2026-08-11.** `go-ride-backend` issues driver tokens with `aud=go-ride-drivers` (`infrastructure/security/jwt.go`, role-specific audience), and both `driver-request-handler` (`JWT_AUDIENCE` default `go-ride-drivers`) and `websocket-gateway` (`JWT_DRIVER_AUDIENCE` default `go-ride-drivers`) validate against the same value — fixed in `go-ride-backend` commit `5e877b4`. This note previously (incorrectly) said `websocket-gateway` expected `aud=go-ride-driver-app`; verified directly against both services' config source and found no mismatch. No action needed before Phase 3.
