...
export function useLoginMutation() {
  return useMutation({
    ...
    onSuccess: (result) => {
      const { access_token, driver } = result;
      useSessionStore.getState().setSession(access_token, driver);
    },
  });
}
...
export function useSignupMutation() {
  return useMutation({
    mutationFn: async (payload: SignupPayload) => {
      // SignupResponse has no token — chain an immediate login with the same
      // credentials to actually authenticate the driver (success criterion 1).
